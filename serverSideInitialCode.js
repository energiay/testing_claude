 ...
 * @param {string} modeId - идентификатор элемента шаблона
 * @return {string}
 */
function getIdCardWithParams(url, modeId) { 
    // получить элемент шаблона по идентификатору
    if (modeId != undefined) {
        return getOverrideWebTemplateId(modeId)
    }
...
function getParam(url) {
    var modeId = OptInt(curOverrideWebTemplateID)
    var oUrl = getObjectUrl(url)
    var sIdOfParams = getIdCardWithParams(oUrl, modeId)
    if (sIdOfParams == '') {
        return {}
    }
